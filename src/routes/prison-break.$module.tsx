        ...
        <div className="mb-10 rounded-2xl border border-[rgba(74,18,89,0.12)] bg-white/80 p-10">
          ...
          <p className="font-serif text-lg leading-relaxed text-[#1F1623]/75 italic">
            {moduleId === "language-detection"
              ? "Every response you've given across all previous modules. The AI reads your words — not your intentions — and surfaces the patterns."
...
function ConversationRunner({
  ...
  const existingArtifact = profile[outputKey as keyof UserProfile] as
    | string
    | undefined;
      ...
      {canGenerate && (
        <div className="mt-8 border-t border-[rgba(74,18,89,0.08)] pt-8">
          <p className="mb-3 font-serif text-sm italic text-[#4A1259]/60">
            Ready to generate your {outputName}? The conversation has given the AI enough to work from.
          </p>
          <button
            onClick={generate}
            disabled={generating}
...
function PhaseOneModule() {
  ...
  return (
    <BrandShell hideStickyCta>
      ...
      {/* Progress strip */}
      <div className="mb-12 grid grid-cols-8 gap-1">
        {phase.modules.map((m) => {
