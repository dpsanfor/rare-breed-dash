...
function Phase2SynthesisRunner({
  ...
  const getSynthesisDescription = () => {
    ...
    if (moduleId === "operating-manual") {
      return "Every artifact from both phases: identity, beliefs, decision filters, standards, leadership philosophy, Magic Gumdrop, Zone of Genius, Category of One, bigger vision, legacy. So that every AI Builder in Rare Breed Club already knows who you are before the first question is asked.";
    }
...
const PHASE2_SAVED_TO = ["The 10X Leap™", "Rare Breed Operating Manual™"];
  ...
  async function initConversation() {
    ...
    try {
      ...
      const greeting = await sendModuleMessage({
        data: { moduleId, messages: [{ role: "user", content: openingNote }] },
      });
    ...
    <BrandShell hideStickyCta>
      ...
      <div className="mb-12 mt-8">
        <div className="mb-4 flex items-center gap-4">
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.3em] text-[#4A1259]/35">
            Module {mod.number}
          </span>
        </div>
